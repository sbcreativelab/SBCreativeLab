...
function Judge({ name, description, image, padding }) {
    let noLinksDescription = description.split(/\[.+?\]\(.+?\)/)
    let links = (description.match(/\[(.+?)\]\((.+?)\)/g) ?? []).map(linkString => /\[(.+?)\]\((.+?)\)/g.exec(linkString).slice(1))
